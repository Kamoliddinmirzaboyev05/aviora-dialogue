...
import { webhookStatusLabels, statusLabel } from "../lib/statusLabels";
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-3 md:flex-row md:items-center">
        ...
        <Link to="/app/simulator"><Button>Xabarni simulyatsiya qilish</Button></Link>
      </div>
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          <CardTitle>Ulanish</CardTitle>
          <div className="mt-4 space-y-3">
            {connections.data?.results.map((connection) => (
              <div key={String(connection.id)} className="rounded-md bg-panel p-3 text-sm">
                ...
                <Badge tone="green">{statusLabel(webhookStatusLabels, String(connection.webhook_status))}</Badge>
              </div>
            ))}
          </div>
        </Card>
        <Card>
          <CardTitle>Kuzatilayotgan chatlar</CardTitle>
          <div className="mt-4 space-y-3">
